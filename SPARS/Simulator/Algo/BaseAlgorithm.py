...
class BaseAlgorithm:
    ...
    def prep_schedule(
        self, new_state, waiting_queue, scheduled_queue, resources_agenda
    ):
        ...
        self.resources_agenda = sorted(
            self.resources_agenda, key=lambda x: (x["release_time"], x["id"])
        )

        self.events = []
        self.compute_speeds = [node["compute_speed"] for node in self.state]
