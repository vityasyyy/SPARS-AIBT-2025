        ...
        for job in waiting_queue:
            if job['res'] <= len(self.available) + len(self.inactive):
                if job['res'] <= len(self.available):
                    ...
                    compute_demand = job['walltime'] * job['res']
                    compute_power = sum(
                        self.compute_speeds[i] for i in reserved_nodes)
                    ...
                    self.JobsManager.add_job_to_scheduled_queue(
                        job['job_id'], reserved_nodes, start_predict_time)
                    self.ResourceManager.reserve_nodes(
                        job['job_id'], reserved_nodes)
            else:
                break
