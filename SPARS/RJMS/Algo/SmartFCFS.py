...
import copy
from re import I
from SPARS.RJMS.Algo.BaseAlgorithm import BaseAlgorithm
...
class SmartFCFS(BaseAlgorithm):
    ...
    def smart_fcfs_switch_on(self):
            ...
            if release_time <= current_time:
                if node_index not in self.ResourceManager.reserved_nodes:
                    ...
                    if node_index in self.inactive:
                        self.future_free_nodes_need_activation.append(
                            node_index)
        ...
        self.future_free_nodes.sort(
            key=lambda node: node in self.future_free_nodes_need_activation)
        need_activation_node = []

        waiting_queue = [
            job for job in self.JobsManager.waiting_queue if job['job_id'] not in self.scheduled]
        for job in waiting_queue:
            if job['res'] <= len(self.future_free_nodes):
                ...
                self.fcfs_predicted.append(
                    {'id': job['job_id'], 'walltime': job['walltime'], 'resources': allocated_resource})
            else:
                break

        if len(need_activation_node) > 0:
            super().push_event(self.current_time, {
                'type': 'switch_on', 'node': need_activation_node})
        ...
        if len(waiting_queue) > 0:
            for job in waiting_queue:
                ...
                if job['res'] <= len(next_releases):
                    ...
                    job_candidates = [
                        r['node_id'] for r in next_releases if r['release_time'] <= job_prediction_start]
                    job_reservation = job_candidates[-job['res']:]

                    next_releases = [
                        nr for nr in next_releases if nr['node_id'] not in job_reservation]

                    # Pre-index machines by id for quick lookup
                    machine_map = {
                        m['id']: m for m in self.ResourceManager.machines}
                    ...
                    for node in switch_off_nodes:
                        ...
                        if switch_off_time is None or switch_on_time is None:
                            raise RuntimeError(
                                f"Node {node} Missing switch_off_time or switch_on_time")
