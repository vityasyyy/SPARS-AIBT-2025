from SPARS.RJMS.ResourceManager import ResourceManager
from SPARS.RJMS.Scheduler import Scheduler
from SPARS.RJMS.JobsManager import JobsManager
...
class RJMS:
    def __init__(self, platform_path, algorithm, start_time, timeout=None):
        ...
        self.Scheduler = Scheduler(
            self.ResourceManager, self.JobsManager, algorithm, start_time, timeout)
        ...
        for _data in simulator_message['event_list']:
            ...
            for event in events:
                if event['type'] == 'arrival':
                    ...
                    self.ResourceManager.allocate(
                        event['nodes'], event['job_id'], event['walltime'], self.current_time)
                    self.JobsManager.remove_job_from_waiting_queue(
                    ...
                    self.ResourceManager.change_dvfs_mode(
