...
jobs_monitor = pd.DataFrame(jobs_monitor)
fault_list = pd.DataFrame(fault_list)

jobs_monitor['allocated_resources'] = jobs_monitor['allocated_resources'].apply(
    lambda x: f' '.join(map(str, x))
)

fault_list.to_csv(f'results/sp/tp/easy_jobs_t{timeout}_predict_fault.csv')
jobs_monitor.to_csv(f'results/sp/tp/easy_jobs_t{timeout}_predict.csv', index=False)
sp_simulator.sim_monitor['nb_res'].to_csv(f'results/sp/tp/easy_host_t{timeout}_predict.csv', index=False)
...
jobs_monitor = jobs_monitor[['job_id', 'allocated_resources', 'starting_time', 'finish_time', 'submission_time']]
jobs_monitor['type'] = 'computing'

final_df = pd.concat([grouped_df, jobs_monitor], ignore_index=True)

final_df = final_df.sort_values(by=['starting_time', 'finish_time'])

calculate_energy(final_df)
calculate_total_time(final_df)
calculate_waiting_time(jobs_monitor)
calculate_last_finish(jobs_monitor)

final_df.to_csv(f'results/sp/tp/easy_nodes_t{timeout}_predict.csv', index=False)
final_df.to_json(f'results/sp/tp/easy_nodes_t{timeout}_predict.json', orient='records', lines=True)