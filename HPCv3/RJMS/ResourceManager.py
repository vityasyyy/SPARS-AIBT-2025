...
class ResourceManager:
        ...
        for node in self.nodes:
            if node['id'] in node_ids:
                if node['state'] == 'switching on':
                    ...
                    raise RuntimeError(f"Node {node['id']} cannot be turned on — state is not switching on")
        ...
        for node in self.nodes:
            if node['id'] in node_ids:
                if node['state'] == 'switching off':
                    ...
                    raise RuntimeError(f"Node {node['id']} cannot be turned off — state is not switching off")
    ...
    def allocate(self, node_ids, job_id, walltime, current_time):
        ...
        compute_power = sum(node['compute_speed'] for node in self.nodes if node['id'] in node_ids)
        ...
        for node in self.nodes:
            if node['id'] in node_ids:
                if node['can_run_jobs']:
                    ...
                    resource_agenda = next((ra for ra in self.resources_agenda if ra['node_id'] == node['id']), None)

                    if resource_agenda:
                        resource_agenda['release_time'] = finish_time
                else:
                    raise RuntimeError(f"Node {node['id']} cannot be allocated for computation — state is not 'idle'") 
