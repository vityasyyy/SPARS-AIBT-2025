...
class Monitor:
    def __init__(self, platform_info, start_time):
        ...
        for i, node in enumerate(platform_info['machines']):
            ...
            for state in node['states'].keys():
                if state == 'active':
                    entry['active_idle'] = {dvfs_mode: 0 for dvfs_mode in node['dvfs_profiles'].keys()}
                    entry['active_compute'] = {dvfs_mode: 0 for dvfs_mode in node['dvfs_profiles'].keys()}
                else:
                    entry[state] = {dvfs_mode: 0 for dvfs_mode in node['dvfs_profiles'].keys()}
            self.states_dur.append(entry)


    def print_energy(self):
        for entry in self.energy:
            print(f"Node {entry['id']}: Energy Consumption = {entry['energy_consumption']}, Energy Waste = {entry['energy_waste']}")
    
    def print_states_dur(self):
        for state_dur in self.states_dur:
            print(f"Node {state_dur['id']}: {state_dur}")
    
    def on_finish(self):
        for node_state in self.nodes_state:
            state_hist = next((h for h in self.states_hist if h['id'] == node_state['id']), None)
    ...
    def record(self, mode, current_time=None, machines=None, record_job_execution=None):
        if mode not in ('before', 'after'):
            raise ValueError(f"Invalid mode '{mode}'. Expected 'before' or 'after'.")

        if mode == 'before':
            if current_time is None:
                raise ValueError("`current_time` is required for mode 'before'.")
    ...
    def update_energy(self, current_time):
        for node in self.nodes_state:
            ...
            energy_entry = next((e for e in self.energy if e['id'] == node_id), None)
            ecr_entry = next((e for e in self.ecr if e['id'] == node_id), None)

            if not energy_entry or not ecr_entry:
                continue

            timespan = node['duration'] + node['start_time'] - energy_entry['last_update']
