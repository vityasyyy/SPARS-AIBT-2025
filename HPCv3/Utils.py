...
def process_node_job_data(nodes_data, jobs):
    ...
    node_intervals_df['start_time'] = _to_float_series(node_intervals_df['start_time'])
    node_intervals_df['finish_time']= _to_float_series(node_intervals_df['finish_time'])
    ...
    jobs_exploded['finish_time'] = _to_float_series(jobs_exploded['finish_time'])
    # keep job_id as nullable int as well
    if 'job_id' in jobs_exploded.columns:
        jobs_exploded['job_id'] = pd.to_numeric(jobs_exploded['job_id'], errors='coerce').astype('Int64')
    ...
    active_merged['job_id'] = active_merged['job_id'].fillna(-1).astype('Int64')

    # --- Non-active: map to negative codes
    non_active_df = non_active_df.assign(
        job_id = non_active_df['state'].map(mapping_non_active).fillna(-1).astype('Int64'),
    ...
    grouped = combined.groupby(
        ...
        nodes=('node_id', lambda x: ' '.join(map(str, sorted([int(v) for v in x.dropna()]))))
    ).reset_index()
